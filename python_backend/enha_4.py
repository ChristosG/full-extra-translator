...
def unify_german_texts(old_text: str, new_text: str) -> str:
    ...
    This is naive. You can expand or use more advanced logic or an LLM-based approach.
    """
    ...
    # See if there's a decent overlap
    ...
    # match.size is the length of the overlap found

    if match.size > 5:
        # unify them by skipping the overlapped portion from new_text
        # e.g. new_text[match.b + match.size:]
...
def refine_german_text(german_text: str) -> str:
    ...
    We keep it simple. If you prefer not to do correction, 
    just return 'german_text' as is.
    """
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Output ONLY the corrected German text — do not add explanations, disclaimers, or any additional commentary."
            )
        },
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "You receive German text, and you must provide an English translation. "
                    "IMPORTANT: Output ONLY the English translation — "
                    "do not add explanations, disclaimers, or statements like 'Here is the translation.' "
                )
            },
...
async def process_buffer():
    ...
    async with buffer_lock:
        ...
        if buffer_is_full or time_since_last >= MIN_TRANSLATION_INTERVAL:
            # 2) Optionally refine the German text to fix obvious mistakes
