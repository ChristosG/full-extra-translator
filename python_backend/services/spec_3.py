#!/usr/bin/env python3
"""
Godlike Audio Preprocessing & Transcription Pipeline

This script records audio (using a VAD trigger) from a virtual sink,
applies advanced preprocessing (adaptive denoising, spectral filtering,
dynamic range compression, and robust overlap alignment), and sends the
processed audio to a Triton-deployed Whisper model for transcription.
Transcriptions are published to Redis.

It also adapts to the current speaker by updating an average RMS value and
rejects chunks that are too short or that contain only noise (or breathing).

A periodic heartbeat is printed so you know that the system is still alive.
...
SKIP_PHRASES = ["Vielen Dank.", "Tschüss.", 'Untertitelung des ZDF, 2020', "Bis zum nächsten Mal."]
...
CURR_OVERLAP_SEC = 0.5      # seconds from start of current chunk
...
def record_audio_and_publish(whisper_prompt: str, language: str, whisper_model_name: str,
    ...
    def finalize_chunk():
        ...
        try:
            ...
            # Wait briefly for the task to complete to avoid over-queuing.
            try:
                future.result(timeout=10)
            except Exception as e:
                logging.error(f"Error waiting for transcription task: {e}")
        finally:
            buffer_bytes = b''
    ...
    async def record():
        try:
            with sd.InputStream(channels=channels, samplerate=sample_rate, dtype=dtype,
                ...
                logging.info("Recording with adaptive VAD and overlap alignment...")
                while True:
